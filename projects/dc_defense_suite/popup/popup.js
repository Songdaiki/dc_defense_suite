...
function applyStatuses(statuses) {
  ...
  applyMonitorLocks(statuses.monitor);
}
...
function applyMonitorLocks(status) {
  const isLocked = Boolean(status?.isRunning);
  const postDom = FEATURE_DOM.post;
  const ipDom = FEATURE_DOM.ip;

  setDisabled(postDom.toggleBtn, isLocked);
  setDisabled(postDom.saveConfigBtn, isLocked);
  setDisabled(ipDom.toggleBtn, isLocked);
  setDisabled(ipDom.saveConfigBtn, isLocked);
  setDisabled(ipDom.releaseBtn, isLocked || ipDom.releaseBtn.disabled);

  getFeatureConfigInputs('post').forEach((input) => setDisabled(input, isLocked));
  getFeatureConfigInputs('ip').forEach((input) => setDisabled(input, isLocked));
}
