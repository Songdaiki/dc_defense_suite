 ...
 */
async function fetchPostListHTML(page = 1) {
 ...
 */
async function classifyPosts(postNos) {
    if (postNos.length === 0) {
        return {
            success: true,
            successCount: 0,
    ...
    const chunks = chunkArray(uniquePostNos, DC_CONFIG.classifyBatchSize);
    const aggregate = {
        successCount: 0,
        failedNos: [],
        messages: [],
    ...
    for (const chunk of chunks) {
        const result = await classifyPostsWithFallback(ciToken, chunk);
...
async function classifyPostsWithFallback(ciToken, postNos) {
    const batchResult = await classifyPostBatch(ciToken, postNos);
    ...
    const leftResult = await classifyPostsWithFallback(ciToken, postNos.slice(0, midpoint));
    const rightResult = await classifyPostsWithFallback(ciToken, postNos.slice(midpoint));
...
async function classifyPostBatch(ciToken, postNos) {
    if (postNos.length === 0) {
        return { success: true, message: '' };
    }

    const url = `${DC_CONFIG.baseUrl}/ajax/minor_manager_board_ajax/chg_headtext_batch`;
    ...
    const bodyParts = [
        ...
        `headtext=${encodeURIComponent(DC_CONFIG.headtext)}`,
    ];
    ...
    const response = await dcFetchWithRetry(url, {
        ...
        headers: {
            ...
            'Referer': `${DC_CONFIG.baseUrl}/mgallery/board/lists/?id=${DC_CONFIG.galleryId}`,
