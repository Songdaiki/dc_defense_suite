...
class Scheduler {
  ...
  async releaseTrackedBans() {
    ...
    const targets = this.activeBans.filter((entry) => isBanActive(entry));
    if (targets.length === 0) {
      this.log('ℹ️ 해제할 활성 차단 내역이 없습니다.');
      await this.saveState();
      return {
        success: true,
        releasedCount: 0,
    ...
    try {
      this.log(`🔍 활성 차단 ${targets.length}건 해제용 내부 ID 탐색 시작`);
    ...
    while (this.isRunning) {
      try {
        ...
        for (let page = startPage; page <= this.config.maxPage; page += 1) {
          ...
          this.log(`📄 도배기 ${page}페이지 로딩...`);
          const html = await fetchTargetListHTML(this.config, page);
          const posts = parseTargetPosts(html, this.config.headtextName);
          const candidates = posts.filter((post) => !this.hasActiveBan(post.no));

          this.log(`📄 ${page}페이지: 유동 ${posts.length}개, 신규 차단 후보 ${candidates.length}개`);
  ...
  hasActiveBan(postNo) {
    ...
    return this.activeBans.some(
      (entry) => isBanActive(entry) && String(entry.postNo) === String(postNo),
    );
  }
