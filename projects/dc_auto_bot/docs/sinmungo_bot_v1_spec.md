...
4. 누적 총합 `>= 100`이면 자동 처리하지 않고 로그만 남김
...
6. `유동 또는 깡계`일 때만 기존 `update_avoid_list(del_chk=1, avoid_hour=6)` 실행
7. 아니면 자동 처리하지 않고 `자동 차단 조건 미충족` 로그만 남김
