...
관리내역(차단/해제 기록)을 주기적으로 순회하면서 **한자(Han)가 들어간 게시글**을 다시 찾아
...
- 따라서 `1 -> detectedMaxPage` 정방향 순회 중 페이지 경계가 밀리면
  ...
  일부 row가 그 사이클에서는 뒤로 밀려 한 번 건너뛰어질 수 있다

이번 기능은 이를 아래처럼 해석한다.

- 같은 row가 다시 보이는 것은 cycle-level `nos[]` dedupe로 막는다
- 새 row 유입으로 한 번 밀려서 빠진 row는 **다음 5시간 사이클에서 다시 잡는 것**을 허용한다

즉 이 기능은 실시간 완전일치 snapshot 보장이 아니라,
**주기적 전체 재순회를 통한 eventual consistency**를 목표로 한다.
...
> 관리내역 끝 페이지를 먼저 알아낸 뒤, `게시글 + Han 제목 + IP형 row`를 끝까지 다시 훑고, 페이지마다 대상 row를 한 번에 묶어 6시간 재차단을 주기적으로 수행한다.
