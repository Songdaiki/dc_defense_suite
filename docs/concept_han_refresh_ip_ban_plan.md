...
### 5. 상태는 필터 조건이며, `차단 중` row만 포함한다

이번 보강안에서는
관리내역 전체가 아니라 **현재 차단 중인 관리내역 row만** 재차단 대상으로 본다.
...
구현 기준은 상태 텍스트 문자열 비교가 아니라
관리내역 row의 `release` 버튼 존재 기반으로 파싱된 `row.isActive`를 사용한다.

즉 상태 조건까지 포함한 최종 기준은:
...
> 관리내역 끝 페이지를 먼저 알아낸 뒤, `게시글 + 차단 중 row + Han 2글자 이상 제목 + IP형 row`를 끝까지 다시 훑고, 페이지마다 대상 row를 한 번에 묶어 6시간 재차단을 주기적으로 수행한다.
