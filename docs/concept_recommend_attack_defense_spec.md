...
2. 개념글 목록에서 실제 게시물 row의 게시물 번호를 최신순으로 최대 20개 추출한다.
...
- 검사 범위:
  - 개념글 목록 page 1의 최신 게시물 20개
- 목록에서 `공지`, `설문`, 숫자 글번호가 아닌 row는 제외하고 **실제 게시물 row만** 센다.
- 즉, DOM 상단 20줄이 아니라 필터링 후 최신 게시물 20개를 snapshot 대상으로 본다.
...
20개 검사 사이에 약 `5초 ± 0.5초`의 간격을 둬서 한 번에 몰아치지 않도록 한다.

1분은 너무 촘촘해서 불필요한 재조회와 중복 시도가 늘 수 있고,
5분 이상은 대응이 늦어질 수 있으므로 1차 기본값은 2분으로 잡는다.
...
- 각 cycle 시작 시 개념글 목록 page 1을 한 번 조회해 대상 postNo 최대 20개를 확정한다.
- 해당 cycle에서는 **처음 확보한 20개 snapshot 기준으로 순차 처리**한다.
...
- [ ] 개념글 목록 page 1에서 게시물 번호 20개 추출 가능
